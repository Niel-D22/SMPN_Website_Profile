...
// ✅ Tidak perlu import — langsung pakai path string dari public/
// Pastikan file ada di: public/Images/profil.jpeg, dll
const cardData = [
  {
    id: 1,
    title: 'Profil Sekolah',
    desc: 'Sejak didirikan, kami membimbing siswa menjadi generasi berkarakter unggul dan siap berkontribusi bagi bangsa.',
    imgUrl: '/Images/profil.jpeg',
    link: '/profil',
  },
  {
    id: 2,
...
const InfoCard = ({ card }) => {
  ...
  return (
    <div
      className="relative w-full flex flex-col rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer shadow-md transition-shadow duration-300 outline-none border bg-white"
      tabIndex={0}
      style={{
      ...
      <img
        ...
        style={{ transform: active ? 'scale(1.10)' : 'scale(1)', opacity: 0.93 }}
      />
      ...
      <div
        ...
        style={{ padding: 'clamp(12px, 5vw, 19px)' }}>
        <h3
      ...
      <div
        className="block lg:hidden absolute inset-0 pointer-events-none rounded-xl border-white border-opacity-20"
        aria-hidden="true"
        style={{
          boxShadow: '0 2px 18px 3px rgba(70,70,95,.12)',
          borderWidth: '2px',
        }}
      />
    </div>
